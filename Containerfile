...
# built into node26, and sharp uses its WebAssembly runtime (added below).
# --frozen-lockfile is load-bearing: unfrozen, pnpm re-resolves upstream's ^ranges
# to newest-on-npm, which can pull releases built against peers papra doesn't ship
# (2026-08-29: @tanstack/ai-anthropic 0.16.9 vs @tanstack/ai 0.39.1 boot crash).
...
# Legacy deploy runs its own install and re-resolves ^ranges to newest-on-npm,
# ignoring the lockfile (this, not the workspace install, is where
# @tanstack/ai-anthropic 0.16.9 kept sneaking back in and crashing boot). Pin the
...
# FreeBSD runtime extras, merged into the deployed tree (workspace stays pristine
# so the frozen lockfile holds — pnpm add there re-resolves upstream pins):
# - @img/sharp-wasm32: sharp has no FreeBSD prebuilt; wasm fallback, pinned to
#   sharp's exact version (guard below).
# - temporal-polyfill: FreeBSD's node26 is built without the Temporal global;
#   the s6 run script preloads it via `node --import`.
# Installed in a scratch dir because npm can't parse the deployed package.json
# (pnpm `catalog:` protocol); tar-merged so existing scoped dirs merge cleanly.
RUN mkdir -p /tmp/extras && cd /tmp/extras && npm init -y >/dev/null && \
    npm install --force @img/sharp-wasm32@0.34.5 temporal-polyfill@0.3.2 && \
    tar cf - -C node_modules . | tar xf - -C /app/production/node_modules && \
    rm -rf /tmp/extras
